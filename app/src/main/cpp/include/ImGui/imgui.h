...
namespace ImGui
{
	...
	IMGUI_API ImGuiStyle*   GetStyleP();
    ...
    IMGUI_API void          StyleColorsMoon(ImGuiStyle* dst = NULL);
    ...
    IMGUI_API void          SetWindoImGuiCond_AlwayswPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    ...
    //自定义小组件
	IMGUI_API bool          SwitchButton(const char* str_id, bool* v);
	//官方小组件
    // Widgets: Main
    ...
    IMGUI_API bool          IsMouseDown(ImGuiMouseButton button);                               // 是否点击鼠标按钮？
    IMGUI_API bool          IsMouseClicked(ImGuiMouseButton button, bool repeat = false);       // 鼠标按钮点击了吗？（从！从下到下）
    IMGUI_API bool          IsMouseReleased(ImGuiMouseButton button);                           // 鼠标按钮释放了吗？（从“下”到“！向下）
    IMGUI_API bool          IsMouseDoubleClicked(ImGuiMouseButton button);                      // 鼠标按钮双击了吗？（请注意，双击也会报告“是一点”点击（） == true）
    IMGUI_API bool          IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max, bool clip = true);// is mouse hovering given bounding rect (in screen space). clipped by current clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
    IMGUI_API bool          IsMousePosValid(const ImVec2* mouse_pos = NULL);                    // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is no mouse available
    IMGUI_API bool          IsAnyMouseDown(float time);                                                   // 是否长按住了鼠标按钮？
    IMGUI_API bool          IsClickandDrag();
    IMGUI_API ImVec2        GetMousePos();                                                      // shortcut to ImGui::GetIO().MousePos provided by user, to be consistent with other calls
    IMGUI_API ImVec2        GetMousePosOnOpeningCurrentPopup();                                 // retrieve mouse position at the time of opening popup we have BeginPopup() into (helper to avoid user backing that value themselves)
    IMGUI_API bool          IsMouseDragging(ImGuiMouseButton button, float lock_threshold = -1.0f);         // 鼠标拖拽？（如果lock_threshold < -1.0f，则使用 io。鼠标德拉格阈值）
    IMGUI_API ImVec2        GetMouseDragDelta(ImGuiMouseButton button = 0, float lock_threshold = -1.0f);   // 在按下鼠标按钮或刚刚释放鼠标按钮时从初始单击位置返回增量。这将被锁定并返回0.0f，直到鼠标至少超过一次距离阈值（如果lock_threshold <-1.0f，则使用io。鼠标德拉格阈值）
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoResize               = 1 << 1,   // 使用右下手柄禁用用户调整大小
    ImGuiWindowFlags_NoMove                 = 1 << 2,   // 禁止用户移动窗口
    ImGuiWindowFlags_NoScrollbar            = 1 << 3,   // 禁用滚动条（窗口仍可使用鼠标或以编程方式滚动）
    ImGuiWindowFlags_NoScrollWithMouse      = 1 << 4,   // 禁用用户使用鼠标滚轮垂直滚动。在子窗口中，鼠标滚轮将转发给父窗口，除非还设置了 NoScrollbar。
    ImGuiWindowFlags_NoCollapse             = 1 << 5,   // 通过双击禁用用户折叠窗口
    ImGuiWindowFlags_AlwaysAutoResize       = 1 << 6,   // 将每个窗口的大小调整为每帧的内容
    ImGuiWindowFlags_NoBackground           = 1 << 7,   // 禁用绘图背景色（窗口 Bg 等）和外边框。类似于使用 SetNextWindowBgAlpha（0.0f）。
    ImGuiWindowFlags_NoSavedSettings        = 1 << 8,   // 切勿加载/保存.ini文件中的设置
    ImGuiWindowFlags_NoMouseInputs          = 1 << 9,   // 禁用捕捉鼠标，悬停测试与直通。
    ImGuiWindowFlags_MenuBar                = 1 << 10,  // 有一个菜单栏
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // 允许显示水平滚动条（默认情况下处于关闭状态）。您可以使用设置下一个窗口内容大小（ImVec2（宽度，0.0f））;在调用 Begin（） 以指定宽度之前。阅读“水平滚动”部分中imgui_demo中的代码。
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // 禁止在从隐藏状态转换到可见状态时对焦
    ImGuiWindowFlags_NoBringToFrontOnFocus  = 1 << 13,  // 禁止在对焦时将窗口置于最前面（例如，单击窗口或以编程方式赋予窗口焦点）
    ImGuiWindowFlags_AlwaysVerticalScrollbar= 1 << 14,  // 始终显示垂直滚动条（即使内容大小.y <大小.y）
    ImGuiWindowFlags_AlwaysHorizontalScrollbar=1<< 15,  // 始终显示水平滚动条（即使内容大小.x <大小.x）
    ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 16,  // 确保子窗口没有边框使用样式。窗口填充（默认情况下，对于无边框的子窗口，忽略，因为更方便）
    ImGuiWindowFlags_NoNavInputs            = 1 << 18,  // 窗口中没有游戏手柄/键盘导航
    ImGuiWindowFlags_NoNavFocus             = 1 << 19,  // 没有焦点朝向带有游戏手柄/键盘导航的窗口（例如，通过CTRL + TAB跳过）
    ImGuiWindowFlags_UnsavedDocument        = 1 << 20,  // 在标题旁边显示一个点。在选项卡/停靠上下文中使用时，在单击 X + 不假定闭包（将等待用户停止提交选项卡）时选择选项卡。否则，在按下 X 时假定关闭，因此，如果您继续提交，选项卡可能会重新出现在选项卡栏的末尾。
    ImGuiWindowFlags_NoNav                  = ImGuiWindowFlags_NoNavInputs | ImGuiWindowFlags_NoNavFocus,
    ImGuiWindowFlags_NoDecoration           = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse,
    ImGuiWindowFlags_NoInputs               = ImGuiWindowFlags_NoMouseInputs | ImGuiWindowFlags_NoNavInputs | ImGuiWindowFlags_NoNavFocus,

    // [Internal]
    ImGuiWindowFlags_NavFlattened           = 1 << 23,  // [BETA] 允许游戏手柄/键盘导航越过父边界到此子项（仅在没有滚动的子项上使用！
...
enum ImGuiCol_
{
    ...
    ImGuiCol_ChildBg,               // 儿童窗口的背景
    ImGuiCol_PopupBg,               // 弹出窗口、菜单、工具提示窗口的背景
    ImGuiCol_Border,
    ImGuiCol_BorderShadow,
    ImGuiCol_FrameBg,               // 复选框、单选按钮、绘图、滑块、文本输入的背景
...
struct ImColor
{
    ...
    ImColor(float r, float g, float b, float a = 1.0f)              { Value.x = r; Value.y = g; Value.z = b; Value.w = a; }ImColor(const ImVec4& col)                                      { Value = col; }
...
struct ImDrawList
{
    ...
    IMGUI_API void AddArc(const ImVec2& center, float radius, const ImVec2& radian, ImU32 col, int num_segments, float thickness);
    IMGUI_API void AddArcFilled(const ImVec2& center, float radius, const ImVec2& radian, ImU32 col, int num_segments);
