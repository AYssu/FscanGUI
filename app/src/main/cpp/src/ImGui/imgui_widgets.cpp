...
bool ImGui::SwitchButton(const char* str_id, bool* v)
{
	...
	InvisibleButton(str_id, ImVec2((width + 8) + label_size.x, height));
	...
	if (IsItemHovered()) {
        ...
        draw_list->AddRectFilled(p, ImVec2(p.x + width, p.y + height), GetColorU32(*v ? colors[ImGuiCol_Button] : ImVec4(0.85f, 0.85f, 0.85f, 0.85f)), height * 0.50f);
	}
    draw_list->AddCircleFilled(ImVec2(p.x + radius + (*v ? 1 : 0) * (width - radius * 4.5f) + (t * 50), p.y + radius), radius - 5.f, IM_COL32(255, 255, 255, 255));
	RenderText(ImVec2(p.x + width * 1.12f,  p.y  ), str_id);
	return IsItemClicked();
}
